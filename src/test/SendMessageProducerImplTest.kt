...
class SendMessageProducerImplTest {
    ...
    fun `когда есть missing notForPaid noAccess found - отправляем нужные сообщения и подтверждаем каждое`() {
        ...
        val idNotPaid = UUID.randomUUID()
        val idNoAccess = UUID.randomUUID()
        val idFound = UUID.randomUUID()

        val payloadMissing = org.mockito.kotlin.mock<RefundPayloadDto>()
        whenever(payloadMissing.orderId).thenReturn(idMissing)
        whenever(payloadMissing.routingKeyStatus).thenReturn("rk.missing")

        val payloadNotPaid = org.mockito.kotlin.mock<RefundPayloadDto>()
        whenever(payloadNotPaid.orderId).thenReturn(idNotPaid)
        whenever(payloadNotPaid.routingKeyStatus).thenReturn("rk.notPaid")

        val payloadNoAccess = org.mockito.kotlin.mock<RefundPayloadDto>()
        whenever(payloadNoAccess.orderId).thenReturn(idNoAccess)
        whenever(payloadNoAccess.routingKeyStatus).thenReturn("rk.noAccess")

        val payloadFound = org.mockito.kotlin.mock<RefundPayloadDto>()
        ...
        whenever(refundErrorMapper.toErrorDto(eq(payloadMissing), eq(RefundErrorReason.ORDER_NOT_FOUND)))
            .thenReturn(errorMissing)

        whenever(refundErrorMapper.toErrorDto(eq(payloadNotPaid), eq(RefundErrorReason.NOT_PAID_FOR)))
        ...
        val sent = payloadCaptor.lastValue
        assertTrue(sent is RefundSuccessDto, "В found должен улетать RefundSuccessDto")
        sent as RefundSuccessDto
        assertEquals(idFound, sent.orderId)
    ...
    fun `sendMessage проставляет заголовки и correlation data`() {
        ...
        val payload = org.mockito.kotlin.mock<Any>()
